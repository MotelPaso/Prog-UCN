"""
Desafio 4
Crear una simulacion de una carrera de robots (A,B,C,D).
Los movimientos estan detallados en un archivo de texto
Si la letra del robot es mayuscula, avanza una casilla.
Si es minuscula, retrocede una casilla.
...
def ordenar_resultados(letras,resultados): 
    ...
    # se puede utilizar un diccionario, pero no esta en el curso.
...
arch = open('fichas.txt', 'r', encoding='utf-8') 
...
for j in range(numCarreras):
    ...
    print('')
    i = 0
    while i < int(cantidades[j]): # Se usa while por no poder utilizar break
        ...
        if longitudTablero-1 in posiciones: # Similar a usar break
            i = int(cantidades[j])
        i += 1 
...
imprimir_resultados(contFinal,victFinal) 
