...
while linea != '': # esto sin funciones habria quedado horrible
    if 'Países' in linea:
        paises,paises1 = crear_paises(linea,arch)
    elif 'Mapa' in linea:
        mapa = crear_matriz_normal(linea,arch) # aqui reutilizo la misma funcion y me ahorro trabajo
...
for viaje in range(len(viajes)):
    ...
    for movimiento in range(1,cantMov[viaje]+1):
        ...
        # encontrar datos en base a la posicion y tiempo
...
print(separador)
puntos3_4_5(visitas,3) # estos puntos hacen algo muy similar, asi que los meti al mismo def
